...
boxedu = boxedu + theme_classic() + theme(axis.text.x = element_text(angle = 45,
                                                   hjust = 1,
                                                   size = 8),axis.title.x = element_blank())
annotate_figure(ggarrange(boxgdp, boxcrime, boxinsure, boxedu),top = text_grob("Main Measures by Region",size= 14,hjust=0.5))
```
...
plotcrime = baseregioncrime + geom_point()+theme_classic()
plotcrime
...
plotgdpcrime = baseregioncrime2 + geom_point()+theme_classic()
plotgdpcrime
```
```{r}
...
plothscrime = baseregioncrime3 + geom_point()+theme_classic()
plothscrime
```
```{r}
crimeplots = annotate_figure(ggarrange(plotcrime,plotgdpcrime,plothscrime),top ="What Causes Crime?")
crimeplots
```
